...
<style>
  ...
  .wizard-nav .badge.active {
    background: #222;
  }

  .wizard-nav .badge.completed {
    background: #28a745;
  }
  ...
  /* Loading state */
  .item-row-main.loading .price-box .spinner {
    display: inline-block !important;
  }

  .item-row-main .price-box .spinner {
    display: none;
  }

  .item-row-main.loading .lineTotal {
    opacity: .5;
  }
  ...
  #itemsContainer tr:only-child .delete-icon {
    visibility: hidden;
  }
</style>
...
<main class="sell-now-page-main">
  <section class="sell-now-section common-section">
    <div class="container">
      ...
      @endif
      @if ($errors->any())
      <div id="serverError" class="alert alert-danger text-danger small mt-2">
        {{ $errors->first() }}
      </div>
      @endif
      ...
      <form id="sellWizardForm" action="{{ route('sell.store') }}" method="post" enctype="multipart/form-data" class="needs-validation login-form" novalidate>
        @csrf
        ...
        <div id="step1" class="wizard-step active">
          ...
          <div class="sell-now-check-btn d-flex align-items-center justify-content-between">
            <div class="form-check">
              <input class="form-check-input" type="checkbox" name="dontKnow" value="1" id="dontKnow" />
                ...
                <div class="col-md-12">
                  <div class="sell-now-main-box d-flex align-items-center justify-content-between">
                    ...
                    <div class="item-value text-end">
                      <h6>£<span id="grandTotal" class="grandTotal">0.00</span></h6>
                      <button class="common-primary-btn" id="goStep2Btn" type="button">
                        <i class="fa-regular fa-hand-point-right"></i> Next
                      </button>
                    </div>
                  </div>
                  <div id="step1Error" class="text-danger mt-2 text-end" style="display:none;"></div>
                </div>
              </div>
            </div>
          </section>
            ...
            <div class="col-md-12">
              <div class="common-form-page h-100 h-custom checkout-page p-0">
                      ...
                      <div class="row common-form-row">
                        <div class="col-lg-12">
                          <div class="common-label-checkbox">
                            ...
                            <div class="form-check">
                              <input class="form-check-input" type="radio" name="checkout_mode" id="modeLogin" value="login">
                              <label class="form-check-label" for="modeLogin">Login</label>
                            </div>
                            ...
                            <div class="col-md-6 login-only">
                              ...
                              <div class="form-outline" data-mdb-input-init>
                                <input type="text" name="phone" class="form-control"
                                  id="validationTooltip03" value="{{ old('mobile') }}" required />
                          ...
                          <div class="row g-3 mt-1" style="justify-content:flex-start;">
                            ...
                            <div class="col-md-6">
                              ...
                              <div class="form-outline" data-mdb-input-init>
                                <input type="email" name="email" class="form-control"
                                  id="validationTooltip04" value="{{ old('email') }}" required autocomplete="off" />
                            ...
                            <div class="col-md-6 auth-only d-none">
                              <!-- <label class="form-label">Password</label>
                              <input type="password" class="form-control" name="password" autocomplete="new-password"> -->
                              <div class="form-outline" data-mdb-input-init>
                                <input type="password" name="password" class="form-control"
                                  id="validationTooltip5" value="{{ old('password') }}" required />
                            ...
                            <div class="col-md-6 reg-only d-none">
                              ...
                              <!-- <div class="form-outline" data-mdb-input-init>
                                ...
                                <input type="password" class="form-control"
                                  id="validationTooltip06" name="password_confirmation" value="{{ old('confirm_password') }}"  />
                                <label for="validationTooltip06"
                                  class="form-label">Confirm Password</label>
                                <div class="invalid-tooltip">Confirm Password is
                                  required</div>
                              </div> -->
                            </div>
                          </div>
                ...
                <!-- <div class="d-flex justify-content-end mt-3">
                    ...
                    <h6>£<span id="grandTotalOtherSec">0.00</span></h6>
                  ...
                  <div class="container px-0">
                    <div class="row">
                      <div class="col-md-12">
                        ...
                        <div id="ajaxError" class="text-danger mt-2 text-end"></div>
                        <div id="step1Error" class="text-danger mt-2 text-end" style="display:none;"></div>
                      </div>
                    </div>
                  </div>
...
<script>
  (function() {
    if (window.__sellNowInitDone) return;
    window.__sellNowInitDone = true;
    ...
    function gotoStep(n) {
      ...
      if (s2) window.scrollTo({
        top: step2.offsetTop - 20,
        behavior: 'smooth'
      });
    ...
    const ITEMS = [{
        ...
        metal: 'gold',
        purities: ['Select Carat', '24ct (99.99%)', '23ct (95.80%)', '22ct (91.6%)', '21ct (87.5%)', '20ct (83.3%)', '18ct (75%)', '14ct (58.5%)', '9ct (37.5%)']
      },
      {
        key: 'gold_bar',
        label: 'Gold Bar',
        metal: 'gold',
        purities: ['Select Carat', '24ct (99.99%)']
      },
      {
        key: 'gold_coin',
        label: 'Gold Coin',
        metal: 'gold',
        purities: ['Select Carat', '24ct (99.99%)', '22ct (91.6%)']
      },
      {
        key: 'silver_jewellery',
        label: 'Silver Jewellery',
        metal: 'silver',
        purities: ['Select Carat', '925 (92.5%)']
      },
      {
        key: 'silver_bar',
        label: 'Silver Bar',
        metal: 'silver',
        purities: ['Select Carat', '999 (99.9%)']
      },
      {
        key: 'silver_coin',
        label: 'Silver Coin',
        metal: 'silver',
        purities: ['Select Carat', '999 (99.9%)', '925 (92.5%)']
      },
      {
        key: 'platinum_jewellery',
        label: 'Platinum Jewellery',
        metal: 'platinum',
        purities: ['Select Carat', '950 (95%)']
      },
      {
        key: 'platinum_bar',
        label: 'Platinum Bar',
        metal: 'platinum',
        purities: ['Select Carat', '999.5 (99.95%)']
      },
      {
        key: 'palladium_bar',
        label: 'Palladium Bar',
      ...
      {
        key: 'palladium_jewellery',
        label: 'Palladium Jewellery',
        metal: 'palladium',
        purities: ['Select Carat', '950 (95%)']
      },
    ];
    ...
    function tplRow(i) {
      return `
    <tr class="item-row-main" data-index="${i}">
    ...
    function bindRow(tr) {
      ...
      tr.querySelector('.purity').addEventListener('change', () => {
        recalcRow(tr);
        saveDraft();
      });
      tr.querySelector('.plusBtn').addEventListener('click', () => {
        const q = tr.querySelector('.qty');
        q.value = Math.min(9999, (+q.value) + 1);
        recalcRow(tr);
        saveDraft();
      });
    ...
    function recomputeTotals() {
      ...
      const grams_v = f2(grams); // your formatter
      // document.querySelector('#totalGrams').textContent = f2(grams);
      document.querySelectorAll('.totalGrams').forEach(el => el.textContent = grams_v);

      //document.querySelector('#grandTotal').textContent = f2(total);
      const total_v = f2(total); // your formatter
      document.querySelectorAll('.grandTotal').forEach(el => el.textContent = total_v);
    ...
    function canProceedToStep2() {
      ...
      for (const tr of rows) {
        ...
        if (!purity || purity.toLowerCase().includes('select')) {
          return {
            ok: false,
            msg: 'Please select a carat for each item.'
          };
        }
        if (weight <= 0) {
          return {
            ok: false,
            msg: 'Please enter a positive weight for each item.'
          };
        }
        anyTotal += lt;
      }
      if (anyTotal <= 0) return {
        ok: false,
        msg: 'We could not price your items. Please review carat/weight.'
      };
      return {
        ok: true
      };
    ...
    const authOnly = document.querySelectorAll('.auth-only'); // register OR login
    const loginOnly = document.querySelectorAll('.login-only');
    const submitCta = document.getElementById('submitCta');


    function applyMode() {
      if (!modeRegister || !modeGuest || !modeLogin) return;
      const isRegister = modeRegister.checked;
      const isLogin = modeLogin.checked;

      regOnly.forEach(el => el.classList.toggle('d-none', !isRegister));
      loginOnly.forEach(el => el.classList.toggle('d-none', isLogin));
      authOnly.forEach(el => el.classList.toggle('d-none', !(isRegister || isLogin)));


      if (submitCta) submitCta.textContent = isRegister ? 'Register & move' : submitCta.textContent = isLogin ? 'Login' : 'Continue as guest';

      setRequired(['first_name', 'phone'], !isLogin);
      saveDraft(2);
    }
    if (modeGuest && modeRegister && modeLogin) {
      modeGuest.addEventListener('change', applyMode);
      modeRegister.addEventListener('change', applyMode);
      modeLogin.addEventListener('change', applyMode);
      applyMode();
    }
    ...
    form.addEventListener('submit', () => {
      ...
      if (fn && nameHidden && !nameHidden.value) {
        const first = (fn.value || '').trim();
        // const last = (ln.value || '').trim();
        nameHidden.value = (first).trim();
      }
      // clear draft after submit
      try {
        localStorage.removeItem('sellWizardDraft');
      } catch (e) {}
    });
    ...
    function restoreDraft() {
      try {
        ...
        if (d.guest) {
          ...
          const ml = document.getElementById('modeLogin');
          if (mg && mr && ml) {
            if (mode === 'register') {
              mr.checked = true;
              mg.checked = false;
        ...
        // step
        if (d.step === 2 &&   d.rows[0].item_key != '') gotoStep(2);
        // if (d.step === 2) gotoStep(2);
        else gotoStep(1);
      } catch (e) {
  ...
  $('#finalSubmitBtn').on('click', function(e) {
    ...
    const $btn = $(this);
    const $form = $('#sellWizardForm');
    const realUrl = $form.attr('action'); // final submit target
    const fd = new FormData($form[0]);

    $.ajax({
      url: checkSellForm, // << use different route here
      type: 'POST',
      data: fd,
      processData: false,
      contentType: false,
      ...
      success: function() {
        // Passed validation — now submit to the REAL action
        // // return false;
        // $form.attr('action', realUrl);
        // $form.off('submit'); // just in case
        $form[0].submit();
      },
      error: function(xhr) {
        if (xhr.status === 422 && xhr.responseJSON?.errors) {
          // const firstMsg = Object.values(xhr.responseJSON.errors)[0][0] || 'Please check your inputs.';
          const errors = xhr.responseJSON.errors;
          const firstKey = Object.keys(errors)[0]; // e.g. "email"
          const firstMsg = errors[firstKey][0] || 'Please check your inputs.';

          $('#ajaxError').text(firstMsg).show();
          $(`[name="${firstKey}"]`).css('border-color', '#dc3545');
          $(`[name="${firstKey}"]`).next('label').css('color', '#dc3545');

        } else {
          $('#ajaxError').text('Something went wrong. Please try again.').show();
        }
      },
      complete: function() {
