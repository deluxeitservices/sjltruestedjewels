...
class CheckoutController extends Controller
{
    public function checkout(Request $r, CartService $cartSvc, PricingService $pricing){
        ...
        $subtotal=0.0; $vat=0.0; $lines=[];
        foreach($cart->items as $ci){
            $p=$ci->product; $unit=$pricing->priceFor($p) ?? 0.0; $line=round($unit*$ci->quantity,2);
            $vatRate=$pricing->vatRateFor($p); $ex=$line/(1+$vatRate);
            $subtotal += $ex; $vat += $line-$ex;
        ...
        $order = Order::create([
            'cart_id'=>$cart->id,'order_no'=>'ORD-'.Str::upper(Str::random(8)),'status'=>'pending','currency'=>'GBP',
            'subtotal_gbp'=>round($subtotal,2),'vat_gbp'=>round($vat,2),'total_gbp'=>$total,
            'customer_email'=>$r->email,'customer_name'=>$r->name,'lock_expires_at'=>now()->addMinutes(10),
        ]);
        foreach($lines as $L){
            OrderItem::create([
                'order_id'=>$order->id,'product_id'=>$L['product_id'],'title'=>$L['title'],'metal'=>$L['metal'],'weight_g'=>$L['weight_g'],
                'qty'=>$L['qty'],'unit_price_gbp'=>$L['unit'],'vat_rate'=>$L['vat_rate'],'vat_gbp'=>round($L['line']-($L['line']/(1+$L['vat_rate'])),2),'line_total_gbp'=>$L['line']
            ]);
        }
        ...
        foreach($order->items as $it){
            $lineItems[]=[
                'price_data'=>[
                    'currency'=>'gbp','product_data'=>['name'=>$it->title],
        ...
        $order->checkout_session_id=$session->id; $order->save();
    ...
    $order->load('items'); // ensure you add a relation in Order model
